...
void Entry::fnPopDialogue()
{
	EntryDialogue testdlg;
	...
	testdlg.fnSetPassword(miPassword);
	...
	if (IDOK == testdlg.DoModal())
	{
		...
		miPassword = testdlg.fnGetPassword();
		miKeyboard = testdlg.fnGetKeyboard();
	}
}

void Entry::fnWriteToFile(CFile * pFile, int Top, int Left)
{
	CString strWrite;
	CString strTemp;
