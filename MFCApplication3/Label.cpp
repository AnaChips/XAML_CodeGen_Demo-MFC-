...
void Label::fnDraw(CDC * pdc)
{
	...
	Pen.CreatePen(PS_SOLID, miBorderWidth, RGB(mbRed, mbGreen, mbBlue));
...
void Label::fnSetBorderWidth(int Width)
{
	miBorderWidth = Width;
}


void Label::fnPopDialogue()
{
	LabelDialogue testdlg;
	...
	if (IDOK == testdlg.DoModal())
	{
		...
		/*CString strTemp;
		strTemp.Format(_T("%d"), miFontFace);
		AfxMessageBox(strTemp);
		*/
	}
}

void Label::fnWriteToFile(CFile * pFile, int Top, int Left)
{
	CString strWrite;
	CString strTemp;
	...
	strTemp.Format(_T("%2x%x%x\""), mbRed, mbGreen, mbBlue);
	strWrite.Append(strTemp);
